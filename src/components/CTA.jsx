  ...
  return (
    <section style={{padding:"clamp(20px,4vw,56px)"}}>
      <div data-reveal style={{maxWidth:1300,margin:"0 auto",position:"relative",borderRadius:40,overflow:"hidden",background:"linear-gradient(135deg,#191919,#242424)",padding:"clamp(60px,9vw,120px) clamp(28px,5vw,80px)",textAlign:"center"}}>
        ...
        <div style={{position:"absolute",inset:0,background:image ? "rgba(10,10,10,.62)" : "radial-gradient(50% 70% at 20% 20%,rgba(3,115,255,.25),transparent 60%),radial-gradient(50% 70% at 80% 80%,rgba(67,147,74,.25),transparent 60%)"}}></div>
        ...
        <div style={{position:"relative"}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:16,justifyContent:"center",marginTop:38}}>
            <button className="mag" onClick={() => go(primaryPage)} style={{background:"#0373ff",color:"#fff",fontWeight:600,fontSize:16,padding:"17px 36px",borderRadius:999,boxShadow:"0 12px 34px rgba(3,115,255,.4)",cursor:"pointer"}}>{renderLabel(primaryLabel)}</button>
